...
/**
 * Client→Server packet decoder (RSProt 237).
 *
 * Wire format per packet:
 *   [u8 ISAAC-scrambled opcode]
 *   [u8 length]   (var-byte:  size == -1)
 *   [u16 length]  (var-short: size == -2)
 *   [payload]
 *
 * [isaac] is the server-side ISAAC decoder instance (seeded from the
 * raw XTEA keys in the login block).  Pass null to skip ISAAC (proxy mode).
 */
    ...
    override fun decode(ctx: ChannelHandlerContext, buf: ByteBuf, out: MutableList<Any>) {
        while (buf.isReadable) {
            if (currentProt == null) {
                ...
                if (prot == null) {
                    logger.warn { "Unknown client opcode 0x${rawOpcode.toString(16)} — closing" }
                    ctx.close(); return
                }
                currentProt = prot
                expectedLength = prot.size  // may be -1 or -2 for variable
            }
...
/**
 * Server→Client packet encoder (RSProt 237).
 *
 * [isaac] is the server-side ISAAC encoder instance (seeded from keys + 50).
 */
